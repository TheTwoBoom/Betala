    ...
    Column(
        ...
        Column(
            ...
            val variantItems = remember {
                listOf(
                    DropDownItemData(
                        Variant.Classic.name, R.drawable.numbers, variant = Variant.Classic
                    ), DropDownItemData(
                        Variant.Killer.name,
                        R.drawable.dice,
                        enabled = false,
                        variant = Variant.Killer
                    ), DropDownItemData(
                        Variant.Chess.name,
                        R.drawable.chess_knight,
                        enabled = false,
            ...
            for (i in 4..25) {
                if (i % 2 == 0 || i % 3 == 0 || sqrt(i, RoundingMode.FLOOR) * sqrt(
                    ...
                    sudokuSizes.add(
                        DropDownItemData(
                            text = "$i * $i", R.drawable.numbers, number = i
                        )
                    )
                }
            }
            ...
            val selectedSize = MyDropdownMenu(
                sudokuSizes, selectedMenuItem = DropDownItemData(
                    text = "9 * 9", R.drawable.numbers, number = 9
                )
            )
            ...
            if (selectedBoxMenuList.size > 1) {
                ...
                if (numbersSqrt * numbersSqrt == numbers) {
                    selectedBoxSize = DropDownItemData(
                        text = "$numbersSqrt * $numbersSqrt", R.drawable.edit, number = numbersSqrt
                    )
...
fun MyOutlinedCard(
    ...
    OutlinedCard(
        modifier = modifier, colors = newColors, border = border
    ) {
        Row(
            modifier = Modifier
                .fillMaxWidth(0.8f)
            ...
            Icon(
                painterResource(itemData.iconRes), contentDescription = itemData.text
            )
            Text(
                modifier = Modifier.padding(start = 6.dp), text = itemData.text
            )
        }
    }
}
...
@Composable
fun MyDropdownMenu(
    menuItems: List<DropDownItemData>,
    key: Any? = null,
    selectedMenuItem: DropDownItemData = menuItems.first()
): DropDownItemData {
    ...
    ExposedDropdownMenuBox(
        ...
        MyOutlinedCard(modifier = Modifier
            .fillMaxWidth()
